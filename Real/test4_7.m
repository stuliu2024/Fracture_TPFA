...
value = zeros(nrf+nlf,1); value(1:nlf) = 1013250; %左边界值1013250 pa，右边界值0
bc.value =value;
...
%加入随机扰动,用来排除  单元中心的影响
% frac_mid_nodes = frac_mid_nodes + (rand(size(frac_mid_nodes,1),size(frac_mid_nodes,2)))*1e-4;
% frac_mid_nodes = (frac_mid_nodes-1)*(1-1e-10)+1;
...
for k=1:size(fractures,1)
    for j=1:fractures(k,11)
        ...
        %求解裂缝中点至局部边1的距离
        distance(1,:) = abs((y1-y2)*x0-(x1-x2)*y0+x1*y2-x2*y1)/sqrt((y1-y2)^2+(x1-x2)^2) + 1e-10*rand(1);
        %求解裂缝中点至局部边3的距离
        distance(3,:) = abs((y3-y4)*x0-(x3-x4)*y0+x3*y4-x4*y3)/sqrt((y3-y4)^2+(x3-x4)^2) + 1e-10*rand(1);
        ...
        %求解裂缝中点至局部边2的距离
        distance(2,:) = abs((y2-y3)*x0-(x2-x3)*y0+x2*y3-x3*y2)/sqrt((y2-y3)^2+(x2-x3)^2) + 1e-10*rand(1);
        %求解裂缝中点至局部边4的距离
        distance(4,:) = abs((y4-y1)*x0-(x4-x1)*y0+x4*y1-x1*y4)/sqrt((y4-y1)^2+(x4-x1)^2) + 1e-10*rand(1);
...
mask2 = ((fracs_global_elem1(:,2) == 0));
virtual_fracs_global_elem1_fix = fracs_global_elem1(~mask2,2);
fracs_global_num_fix = fracs_global_num(~mask2);
face_test = reshape(G.cells.faces((virtual_fracs_global_elem1_fix-1)*4 + [1:4],1),[],4);
virtual_local_num1 = arrayfun(@(row)find(face_test(row,:) == fracs_global_num_fix(row),1),1:size(face_test,1))';
...
virtual_fracs_info1 = [virtual_fracs_global_elem1_fix, virtual_local_num1, actual_fracs_info1(~mask2,4)];
...
% nodefracs1 = cell(size(nodefracs,1),1);
...
%     nodefracs1{i} = unique(nodefracs{i},'rows');
...
filename1 = ['Real_', num2str(nx),'_Solution'];
saveas(gcf, filename1,'epsc');

filename = ['Data_', num2str(nx)];
save(filename,'uc','dof','elem','node','Cpu_time','matrix_elements','fracture_elements','sparsity','conditionalnumber','Points','T','P','fracXAndPCell');


%slice
[x_rect, y_rect] = meshgrid(linspace(xa, xb, nx), linspace(yc, yd, ny));
% y=500
[X_slice1_51, Y_slice1_51] = meshgrid(linspace(xa, xb, nx),500);
...
%x=625
[X_slice1_52, Y_slice1_52] = meshgrid(625,linspace(yc, yd, ny));
R_slice1_52 = sqrt((X_slice1_52 - X_slice1_52(1)).^2+(Y_slice1_52-Y_slice1_52(1)).^2);
U_slice1_52 = interp2(x_rect,y_rect,reshape(p,nx,ny)',X_slice1_52,Y_slice1_52,'linear');
save(filename,'X_slice1_52','Y_slice1_52','R_slice1_52','U_slice1_52','-append');
figure;plot(R_slice1_52,U_slice1_52,'LineWidth',1.5);
