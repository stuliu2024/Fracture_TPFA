...
[T_fracs,posfracs] = my_computeTrans_fracs_3D1(G);
...
xlim([xmin,xmax]); ylim([ymin,ymax]); zlim([zmin,zmax]);
%% function
function [T_fracs,posfracs] = my_computeTrans_fracs_3D1(G)
xmin = 0; xmax = 1; ymin = 0; ymax = 1; zmin = 0; zmax =1;
Cell_N = G.cartDims(1); Cell_M = G.cartDims(2); Cell_L = G.cartDims(3);
...
ParametersofFracture(:,3:14) = [1/2,1/3,4/5,  1/5,1/3,1/5,   1/5,2/3,1/5,   1/2,2/3,4/5;...
                              ...
                              + (rand(NumberofFractures,12)-0.5)*1e-2; % position of fractures
...
vvt = [];
eplisonkf = [];
for i=1:NumberofFractures
    sigma_fracs = ParametersofFracture(i,15:17);
    sst = sigma_fracs'*sigma_fracs;  %sigma*sigmaT
    vvt_temp = eye(size(G.cartDims,2)) - sst; % I - sigma*sigmaT
    vvt = [vvt;repmat([vvt_temp(1,1) vvt_temp(1,2) vvt_temp(1,3) vvt_temp(2,2) vvt_temp(2,3) vvt_temp(3,3)],ParametersofFracture(i,18)*6,1)];
    eplisonkf = [eplisonkf;repmat(ParametersofFracture(i,1).*ParametersofFracture(i,2),ParametersofFracture(i,18)*6,1)];
    %1./abs(v*n);
    v_vel_temp = sqrt([vvt_temp(1,1) vvt_temp(2,2) vvt_temp(3,3)]);   
    v_vel_temp = repmat(v_vel_temp,ParametersofFracture(i,18)*6,1);
    v_vel = [v_vel;v_vel_temp];
end

kfvn = 1./abs(v_vel(:,1).*N_fracs(:,1)+v_vel(:,2).*N_fracs(:,2)+v_vel(:,3).*N_fracs(:,3));
kfvvt = kfvn.*eplisonkf.*vvt;
