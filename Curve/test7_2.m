...
y_partition = (16*sin(t_partition).^3)/50+1/2;  %裂缝参数方程
...
for ell = 1:length(ele_passed)
    ...
    x_sampling = -(13*cos(t_sampling)-5*cos(2*t_sampling)-2*cos(3*t_sampling)-cos(4*t_sampling))/50+1/2;           %每个单元采样点x坐标
    y_sampling = (16*sin(t_sampling).^3)/50+1/2;        %每个单元采用点y坐标
    ...
    permeabilityfractures(sum(numberoffracturespassed(1:numfractures-1,1)) + ell,1:2) = 1e7 * ones(size(t_sampling));
    thicknessfractures(sum(numberoffracturespassed(1:numfractures-1,1)) + ell,1:2) = 0.005 * ones(size(t_sampling));
    
    lengthfractures(sum(numberoffracturespassed(1:numfractures-1,1)) + ell,1) = time_out - time_in + 1e-30;
