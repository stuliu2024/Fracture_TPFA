...
% 根据test7_3结果，我们得知，如果是四条边通量进行计算，则结果较差，如果我们采用仅两条边通量进行计算，
% 采用偶数网格，直接拆分成直线进行模拟，试试结果怎么样？

%对裂缝中心坐标加了扰动，结果较为稳定，偶数剖分也能表现出裂缝
...
%加入扰动，降低风险
x_center1 = 3/8+(rand-3/8)*1e-3; 
x_center2 = 5/8+(rand-5/8)*1e-3; 
y_center = 1/2+(rand-1/2)*1e-3;  %裂缝参数，（x_center,y_center）即为中心坐标
x_partition = (x_center1-1/8*cos(t_partition)).*(t_partition<=pi) + (x_center2+1/8*cos(t_partition)).*(t_partition>pi);
y_partition = y_center-1/8*sin(t_partition);   %裂缝参数方程
...
for ell = 1:length(ele_passed)
    ...
    if ell==1  %弯曲裂缝经过初始单元
    time_in = t_start;
    time_out = t_partition(where_inter(ell+1)-1);
    elseif ell == length(ele_passed) %弯曲裂缝经过终止单元
    time_in = t_partition(where_inter(ell));
    time_out = t_end;
    else   %其余裂缝经过单元
    time_in = t_partition(where_inter(ell));
    time_out = t_partition(where_inter(ell+1)-1);
    ...
    x_sampling = (x_center1-1/8*cos(t_sampling)).*(t_sampling<=pi)+(x_center2+1/8*cos(t_sampling)).*(t_sampling>pi);           %每个单元采样点x坐标
    y_sampling = y_center-1/8*sin(t_sampling);       %每个单元采用点y坐标
...
%     lengthfractures(sum(numberoffracturespassed(1:numfractures-1,1)) + ell,1) = sqrt((x_sampling(1)-x_sampling(2))^2 + (y_sampling(1)-y_sampling(2))^2);
end

%排除长度为零的单元
mask_length = (lengthfractures == 0);
ele_passed = ele_passed(~mask_length,:);
permeabilityfractures = permeabilityfractures(~mask_length,:);
thicknessfractures = thicknessfractures(~mask_length,:);
lengthfractures = lengthfractures(~mask_length,:);
frac_nodes = frac_nodes(~rldecode(mask_length,2,1),:);
pathfractures = pathfractures(~mask_length,:);
v = v(~mask_length,:);
theta_avg_sum = theta_avg_sum(~mask_length,:);
...
%加入随机扰动,用来排除  单元中心的影响
frac_mid_nodes = frac_mid_nodes + (rand(size(frac_mid_nodes,1),size(frac_mid_nodes,2))- frac_mid_nodes)*1e-9;
...
%测试，假设局部为横平竖直
% actual_fracs_flux_info2(:,4)=1;
% actual_fracs_flux_info2(:,[6 7]) = repmat([1 0;1 0;0 1;0 1],size(actual_fracs_flux_info2,1)/4,1);
...
view([50 30]);
filename = ['Curve_',num2str(nnn),'_solution_2'];
...
filename1 = ['Curve_',num2str(nnn),'_2'];
save(filename1);
